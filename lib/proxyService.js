...
class ProxyService {
    ...
    async testProxy(proxy, protocol) {
        ...
        try {
            let curlCommand;
            const timeout = 3; // 3 second timeout for curl

            // Build curl command based on protocol
            switch (protocol) {
                case 'http':
                case 'https':
                    curlCommand = `curl -x http://${proxy} https://httpbin.org/ip -k --connect-timeout ${timeout} --max-time ${timeout} -s`;
                    break;
                case 'socks4':
                    curlCommand = `curl https://httpbin.org/ip --socks4 ${proxy} --connect-timeout ${timeout} --max-time ${timeout} -s`;
                    break;
                case 'socks5':
                    curlCommand = `curl https://httpbin.org/ip --socks5 ${proxy} --connect-timeout ${timeout} --max-time ${timeout} -s`;
                    break;
                default:
            ...
            try {
                ...
                returnedIP = response.origin || response.ip;
            } catch (parseError) {
                // If JSON parsing fails, try to extract IP from text response
    ...
    async saveWorkingProxies(workingProxies) {
        ...
        const workingMetadata = {
            ...
            workingCounts: Object.fromEntries(
                Object.entries(workingProxies).map(([protocol, list]) => [protocol, list.length])
            ),
    ...
    // Get service status
    getStatus() {
        const totalWorking = Object.values(this.workingProxies)
            .reduce((sum, proxies) => sum + (proxies?.length || 0), 0);

        return {
            isInitialized: this.lastUpdate !== null,
            lastUpdate: this.lastUpdate,
            totalWorkingProxies: totalWorking,
            workingProxiesByProtocol: Object.fromEntries(
