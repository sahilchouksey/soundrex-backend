...
class ProxyService {
    constructor() {
        ...
        this.proxyTestTimeout = parseInt(process.env.PROXY_TEST_TIMEOUT) || 5000;
        ...
        this.quickInitMode = process.env.PROXY_QUICK_INIT === 'true';
        this.backgroundInitMode = process.env.PROXY_BACKGROUND_INIT === 'true';
        this.maxConcurrent = parseInt(process.env.PROXY_MAX_CONCURRENT) || 3;
    ...
    async initialize() {
        ...
        const totalAvailable = Object.values(this.workingProxies).reduce((sum, arr) => sum + arr.length, 0);
        
        // If we have no proxies at all, try downloading them
        if (totalAvailable === 0) {
            console.log('🔄 No proxies available, downloading fresh proxy lists...');
            try {
                await this.downloadProxies();
                await this.loadProxiesFromFiles();
            } catch (error) {
                console.warn('⚠️ Failed to download proxies, will continue without them:', error.message);
            }
        }

        // For server startup, we'll initialize with minimal testing to avoid restarts
        console.log('⚡ Server startup mode - minimal proxy testing to avoid restarts');
        
        // Schedule proxy testing as a background task after a delay
        this.scheduleBackgroundTesting();

        this.lastUpdate = Date.now();
        console.log('✅ Proxy Service initialized successfully (background testing scheduled)');
        return this.getStatus();
    }

    // Schedule background proxy testing to avoid server restarts
    scheduleBackgroundTesting() {
        // Wait 10 seconds after server start to begin testing
        setTimeout(async () => {
            try {
                console.log('🔍 Starting background proxy testing...');
                
                // Test a small number of proxies quietly
                const workingProxies = await this.testProxies(2, 5, true);
                const totalWorking = Object.values(workingProxies).reduce((sum, arr) => sum + arr.length, 0);
                
                if (totalWorking > 0) {
                    console.log(`✅ Background testing found ${totalWorking} working proxies`);
                } else {
                    console.log('⚠️ Background testing found no working proxies');
                    
                    // If no working proxies found, schedule a more thorough test later
                    setTimeout(() => {
                        this.scheduleFullTesting();
                    }, 60000); // Test more thoroughly after 1 minute
                }
                
            } catch (error) {
                console.warn('⚠️ Background proxy testing failed:', error.message);
            }
        }, 10000); // 10 second delay
    }

    // Schedule full proxy testing (used when background testing finds nothing)
    scheduleFullTesting() {
        setTimeout(async () => {
            try {
                console.log('🔍 Starting full proxy testing...');
                await this.testProxies(3, 20, false);
                
                const totalWorking = Object.values(this.workingProxies).reduce((sum, arr) => sum + arr.length, 0);
                console.log(`📊 Full testing complete: ${totalWorking} working proxies found`);
                
            } catch (error) {
                console.warn('⚠️ Full proxy testing failed:', error.message);
            }
